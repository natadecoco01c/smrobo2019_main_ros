...
enum class fullOP
	: uint8_t
	{
		standby, start, sz_to_tp, launch, reload,
//	tp_to_sz,
	release_reloader,
	wait_next_throw,
...
Mr1Main::Mr1Main(void) {
	...
	nh_.param("/main/slipper_detect_threshold", this->slipper_detect_threshold, 40);
...
void Mr1Main::control_timer_callback(const ros::TimerEvent &event) { //最初はlauncherは下がった状態　戻す奴は上
	...
	&& !this->operating) {
		...
		//launchにするのはslipperloadedcallbackの中
		//なんか動かす奴
